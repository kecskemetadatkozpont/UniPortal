-- ============================================================================
-- RUN_ALL_83.sql — a validált oktatói lista betöltése a törzsbe (2026-09-24)
-- ============================================================================
-- MIÉRT KELL EZ MÉG: a 82 lefutott, és a törzsfeltöltés az oktatói
-- nyilvántartásból MÉRVE 64 kutatót vitt be. A validált lista viszont 273
-- nevet tartalmaz — tehát 209 személy nincs benne aktívan a nyilvántartásban,
-- és őket máshonnan kell felvinni.
--
-- MIT TESZ: ad egy service_role-only, ÚJRAFUTTATHATÓ importot
-- (`grants_researcher_import`), plusz egy `kulso_kod` mezőt (az NJE-kód), hogy
-- egy későbbi lista ugyanazt a személyt találja meg, és ne hozzon létre
-- másodszor. Az egyeztetés sorrendje: intézményi kód → oktatói nyilvántartás
-- kódja → EGYEDI névegyezés; több azonos nevűnél nem dönt, hanem jelenti.
--
-- AMIT SZÁNDÉKOSAN NEM TESZ: az echo.teacher-hez NEM ír és senkit nem aktivál
-- benne — arra az ECHO épül, és erről nem itt kell döntést hozni. A
-- helykitöltő @nje-import.invalid e-mailt nem írja be.
--
-- A FUTÁS VÉGÉN EZT KELL LÁTNOD:
--   NOTICE: Rendben: 83 — validalt lista importja (service_role), intezmenyi kod a torzsben.
-- Az első futáson ez is megjelenik előtte (a listafüggvény kiegészítése),
-- újrafuttatáskor már nem:
--   NOTICE: A kutatoi lista mostantol az intezmenyi kodot is visszaadja.
--
-- Ha bármelyik utasítás hibára fut, a SQL Editor az EGÉSZ csomagot visszavonja.
--
-- FUTÁS UTÁN: szólj, és feltöltöm a 273 nevet (a lista a gépemen lévő
-- munkapéldányból megy át, a repóba nem került és nem is kerül bele).
-- ============================================================================


-- ####################################################################
-- ### 83_grants_roster_import.sql
-- ####################################################################
