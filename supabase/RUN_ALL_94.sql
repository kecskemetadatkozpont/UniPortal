-- ============================================================================
-- RUN_ALL_94.sql — „kit melyik felhívásra" gépi olvasóval
-- ============================================================================
-- MIÉRT: a találatok eddig csak az irodai képernyőn látszottak, mert minden
-- olvasó RPC bejelentkezett felhasználót kíván. Ezért nem tudtam számokkal
-- megmutatni, kit melyik felhívásra érdemes felkérni — és ezért nem tudja a
-- heti összefoglaló sem kiküldeni.
--
-- MIT AD:
--   grants_match_report_etl() — felhívásonként az arculatok, arculatonként a
--       legjobb jelöltek: név, kar, összpontszám és a komponensek, melyik úton
--       (beágyazás vagy szóegyezés), van-e angol nyelvű kimenete, kértük-e már
--       fel valaha, és MELYIK MŰVE alapján javasoljuk.
--   grants_team_suggest_etl() — a csapatajánló gépi változata, hogy az
--       ütemezett kör a 2-3 csapatváltozatot is elő tudja készíteni.
--
-- ADATVÉDELEM: a jelentés neveket ad vissza, ezért KIZÁRÓLAG a service_role
-- hívhatja. A bejelentkezett felhasználó továbbra is a saját jogosultsága
-- szerinti irodai nézetet látja — ezen a szkript nem változtat.
--
-- MIT VÁRJ A FUTÁS VÉGÉN:
--   NOTICE: Rendben: 94 — szolgaltatasi jelentes es csapatajanlo (csak service_role).
--   NOTICE: Rendben: az ECHO bekuldes tovabbra is zart.
--
-- FUTÁS UTÁN: szólj, és kiírom a listát felhívásonként.
-- ============================================================================



-- ####################################################################
-- ### 94_grants_report.sql
-- ####################################################################
